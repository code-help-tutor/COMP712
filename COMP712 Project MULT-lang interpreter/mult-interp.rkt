...
;;;========= Primitives =========
(define (apply-primitive prim val1 val2)
  (cases primitive prim
    [add-prim () (num-val (+ val1 val2))]
    [subtract-prim () (num-val (- val1 val2))]
    (mult-prim () (num-val (* val1 val2)))
))
...
(define value-of
  (lambda (exp env)
    (cases expression exp
      ...
      [var-exp (var) (lookup-env env var)]
;      [diff-exp (exp1 exp2) (let ([val1 (value-of exp1 env)]
;                                  [val2 (value-of exp2 env)])
;                              (let ([num1 (expval->num val1)]
;                                    [num2 (expval->num val2)])
;                                (num-val (- num1 num2))))]
      [primcall-exp (prim args)
                    (let ([val1 (value-of (car args) env)]
                          [val2 (value-of (cadr args) env)])
                      (let ([num1 (expval->num val1)]
                            [num2 (expval->num val2)])
                        (apply-primitive prim num1 num2)))]
